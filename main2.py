...
def recommend_items(fav_item, rules_df, top_n=5):
    fav_item = fav_item.lower()
    
    filtered_rules = rules_df[rules_df['antecedents'].apply(
        lambda x: fav_item in [item.lower() for item in x]
    )]

    if filtered_rules.empty:
        return pd.DataFrame([{"Message": f"No strong recommendations found for '{fav_item}'"}])
